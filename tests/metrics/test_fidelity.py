...
from ..utils import generate_model, generate_regression_model, generate_data, almost_equal
...
def test_data_types_and_operators():
    # ensure that the different metrics are robust to several data types and operators
    ...
    for task, operator in operators.items():
        score_insertion = Insertion(
            model, x, y, operator=operator, steps=3,
        ...
        score_mufidelity = MuFidelity(
            model, x, y, operator=operator, nb_samples=3
        ).evaluate(explanations)
